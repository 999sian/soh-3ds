...
class Archive : public std::enable_shared_from_this<Archive> {
  ...
  public:
     ...
     */
    /**
     * @brief Looks up the indexed path for a hash, or nullptr when absent.
     *
  ...
  protected:
     ...
     *
     * Archive implementations that learn their entry count up front (e.g. a ZIP
     * directory) call this before IndexFile() to avoid repeated rehashing while
     * a large archive is indexed.
     */
    void ReserveIndex(size_t expectedFileCount);
