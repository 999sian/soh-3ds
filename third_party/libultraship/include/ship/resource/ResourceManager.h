...
class ResourceManager {
  ...
  public:
    ...
    /** @brief Number of entries in the resource cache (live resources + negative lines). */
    size_t GetResourceCacheSize();

#ifdef __3DS__
    /**
     * @brief Cache size, and how many lines the cache solely owns.
     * @param unreferenced Out: lines with use_count 1 (reclaimable); may be nullptr.
     * @return Total number of cache entries.
     */
    size_t Soh3dsCacheStats(unsigned* unreferenced);

    /** @brief Formats a per-top-level-directory `prefix=total/free` breakdown into @p out. */
    void Soh3dsCacheReport(char* out, unsigned size);

    /**
     * @brief Distinct second path segments of cached entries under @p prefix.
     *
     * `objects/object_foo/bar` under prefix "objects/" yields "object_foo".
     * Lets a caller act on what is actually cached instead of probing every
     * name in the game's object table against the archive index.
     */
    std::vector<std::string> Soh3dsCachedSubdirs(const std::string& prefix);
#endif
