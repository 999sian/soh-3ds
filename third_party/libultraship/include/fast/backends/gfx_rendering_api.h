...
class GfxRenderingAPI {
  public:
    ...
    // Backends that can lose a texture's storage under memory pressure (3DS
    // linear heap) return false so the interpreter re-imports the texture
    // instead of drawing an empty slot. Default: storage never goes away.
    virtual bool TextureHasStorage(uint32_t texId) {
        (void) texId;
        return true;
    }
