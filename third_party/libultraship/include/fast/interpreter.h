...
// SoH-3DS: GfxSpTri1 re-derives the combiner, shader program, clamp mode and
// texture scale factors for every triangle from RDP/RSP state that changes
// only per draw (~10% of triangles). Everything those derivations read is
// copied into this key; while it compares equal the derived values are reused.
struct TriStateKey {
    uint64_t combine_mode;
    uint32_t other_mode_l, other_mode_h, geometry_mode, extra_geometry_mode;
    int32_t shader_id;
    ...
    struct Tile {
        float uls, ult, lrs, lrt;
        uint32_t line_size_bytes;
        uint8_t siz, cms, cmt, masks, maskt, shifts, shiftt, tmem_index;
        bool operator==(const Tile&) const noexcept = default;
    } tile[2];
    struct Loaded {
        uint32_t orig_size_bytes, size_bytes, full_image_line_size_bytes, line_size_bytes;
        float h_byte_scale, v_pixel_scale;
        bool operator==(const Loaded&) const noexcept = default;
    } loaded[2];
    uint8_t first_tile_index, depth_test_and_mask;
    bool grayscale, decal_mode, alpha_blend;
    bool masked[2], blended[2], textures_changed[2];

    bool operator==(const TriStateKey&) const noexcept = default;
};
...
struct TriStateCache {
    ...
    uint8_t numInputs;
    bool usedTextures[2];
    bool use_alpha, use_fog, use_blend_color, use_grayscale, linear_filter;
...
class Interpreter {
  public:
    ...
    // Internal paired-handler entry: true only after an equality hit emitted
    // without a capacity flush. A token must never survive its TRI2/quad command.
    bool GfxSpTri1Impl(uint8_t vtx1Idx, uint8_t vtx2Idx, uint8_t vtx3Idx, bool isRect, bool reuse);
    ...
    // SoH-3DS: G_SETTIMG_OTR_HASH resolves the same hashes every frame, and
    // each resolution is HashToString + path copy + mutex + cache find (~30 us
    // on ARM11, ~170 per frame). weak_ptr: no ownership, so scene eviction
    // still frees the texture and the next lookup simply misses.
    std::unordered_map<uint64_t, std::weak_ptr<Ship::IResource>> mTextureByHash;
