...
# SoH-3DS: the 3DS has no OpenGL and does not use SDL2 for video, so neither of
# these backends can compile (imgui_impl_opengl3 needs dlfcn.h, imgui_impl_sdl2
# needs SDL.h on the include path). The 3DS port supplies its own citro3d ImGui
# backend instead.
if(NOT N3DS)
target_sources(ImGui
    PRIVATE
...
set(STB_CACHED_SHA256 "")
if(EXISTS "${STB_DIR}/stb_image.h")
    file(SHA256 "${STB_DIR}/stb_image.h" STB_CACHED_SHA256)
endif()
if(NOT STB_CACHED_SHA256 STREQUAL STB_IMAGE_SHA256)
    # Reconfigure must work offline with a verified cache. Download to a
    # temporary file so a failed transfer cannot overwrite the cached header.
    file(MAKE_DIRECTORY "${STB_DIR}")
    set(STB_DOWNLOAD_FILE "${STB_DIR}/stb_image.h.download")
    file(DOWNLOAD "https://raw.githubusercontent.com/nothings/stb/0bc88af4de5fb022db643c2d8e549a0927749354/stb_image.h"
        "${STB_DOWNLOAD_FILE}" STATUS STB_DOWNLOAD_STATUS
        TLS_VERIFY ON TIMEOUT 60 INACTIVITY_TIMEOUT 15)
    ...
    if(NOT STB_DOWNLOAD_CODE EQUAL 0)
        ...
        file(REMOVE "${STB_DOWNLOAD_FILE}")
        message(FATAL_ERROR "Failed to download pinned stb_image.h: ${STB_DOWNLOAD_ERROR}")
    endif()
    file(SHA256 "${STB_DOWNLOAD_FILE}" STB_DOWNLOADED_SHA256)
    if(NOT STB_DOWNLOADED_SHA256 STREQUAL STB_IMAGE_SHA256)
        file(REMOVE "${STB_DOWNLOAD_FILE}")
        message(FATAL_ERROR "Invalid stb_image.h SHA256: expected ${STB_IMAGE_SHA256}, got ${STB_DOWNLOADED_SHA256}")
    endif()
    file(RENAME "${STB_DOWNLOAD_FILE}" "${STB_DIR}/stb_image.h")
endif()
