...
std::shared_ptr<ResourceInitData> ResourceLoader::ResolveMetaAlias(const std::string& filePath,
    ...
    // SoH-3DS: every resource load probed "<path>.meta" - a string concat, a
    // CRC64 over the path and an index lookup per load, on a scene transition
    // that is thousands of loads. Vanilla oot.o2r and soh.o2r contain ZERO
    // .meta entries, so the probe always missed. Ask the index once per
    // archive-set change instead; a mod archive that does ship aliases turns
    // the probe straight back on.
    {
        static size_t sCheckedArchiveCount = SIZE_MAX;
        static bool sAnyMeta = true;
        auto archives = archiveManager->GetArchives();
        const size_t count = archives != nullptr ? archives->size() : 0;
        if (count != sCheckedArchiveCount) {
            sCheckedArchiveCount = count;
            auto metaFiles = archiveManager->ListFiles("*.meta");
            sAnyMeta = metaFiles == nullptr || !metaFiles->empty();
        }
        if (!sAnyMeta) {
            return nullptr;
        }
