...
// SoH-3DS: GetConsoleVariables() returns a shared_ptr by value - an atomic
// refcount pair per call, on top of the map lookup, for the hundreds of C-side
// CVarGet* calls in each game tick. The subsystem lives as long as the
// Context; hold a raw pointer, refreshed if the Context is ever recreated.
static Ship::ConsoleVariable* CVars() {
    static Ship::Context* sContext = nullptr;
    static Ship::ConsoleVariable* sVars = nullptr;
    Ship::Context* context = Ship::Context::GetRawInstance();
    if (context != sContext || sVars == nullptr) {
        sContext = context;
        sVars = context != nullptr ? context->GetConsoleVariables().get() : nullptr;
    }
    return sVars;
}
