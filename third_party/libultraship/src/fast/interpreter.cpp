...
#ifdef __3DS__
// SoH-3DS: <3ds.h> redefines u32/s32 in conflict with libultra's typedefs, so
// declare the libctru query used by the texture cache with a matching ABI.
...
// Bounded 3DS trial candidate; override with 0 for the matched control build.
// Physical-hardware acceptance remains pending; other platforms stay generic.
#ifndef SOH3DS_TRIANGLE_PAIR_REUSE
#ifdef __3DS__
#define SOH3DS_TRIANGLE_PAIR_REUSE 1
#else
#define SOH3DS_TRIANGLE_PAIR_REUSE 0
#endif
#endif
...
void Interpreter::TextureCacheClear() {
    mTriState.valid = false; // mTextures nodes die with the map
...
bool Interpreter::TextureCacheLookup(int i, const TextureCacheKey& key) {
    ...
    auto evict = [this](TextureCacheMap::iterator victim, bool freeStorage) {
        if (freeStorage) {
            mRapi->DeleteTexture(victim->second.texture_id);
        }
        mTextureCache.free_texture_ids.push_back(victim->second.texture_id);
        for (int j = 0; j < SHADER_MAX_TEXTURES; j++) {
            if (mRenderingState.mTextures[j] == &*victim)
                mRenderingState.mTextures[j] = nullptr;
        }
        mTextureCache.lru.erase(victim->second.lru_location);
        mTextureCache.map.erase(victim);
    };

    if (it != mTextureCache.map.end()) {
#ifdef __3DS__
        // SoH-3DS: the backend drops a texture's storage instead of throwing when
        // linear memory runs out. Such an entry must be a miss, or the texture
        // stays invisible until the LRU happens to recycle it.
        if (!mRapi->TextureHasStorage(it->second.texture_id)) {
            evict(it, false);
    ...
    if (mTextureCache.map.size() >= TEXTURE_CACHE_MAX_SIZE) {
        ...
        evict(mTextureCache.lru.front().it, false);
    }
#ifdef __3DS__
    // SoH-3DS: the cap above is count-based; the binding limit on 3DS is linear
    // memory (RGBA8 backings, up to 512 KiB for a prerendered background).
    // Evict least-recently-used entries and free their storage now until that
    // worst case fits. Bounded: storage a draw recorded this frame still reads
    // is only retired by the backend, not freed, so the loop may not converge.
    constexpr uint32_t kLinearReserve = 1024 * 1024;
    for (int evicted = 0; evicted < 16 && !mTextureCache.lru.empty() && linearSpaceFree() < kLinearReserve;
         ++evicted) {
        evict(mTextureCache.lru.front().it, true);
    }
#endif
...
#ifdef __3DS__
// Temporary hardware investigation: an explicit flag enables a bounded trace.
...
static bool IsTempleCarpet(const RawTexMetadata* metadata) {
    return metadata->resource != nullptr &&
           metadata->resource->GetInitData()->Path.find("tokinoma_sceneTex_00CFA0") != std::string::npos;
}

static void TraceTextureImport(const RDP* rdp, const RawTexMetadata* metadata, int unit, int tile,
                               const uint8_t* address, uint32_t textureId, bool hit, bool replacement) {
    FILE* trace = TextureTraceFile();
    if (trace == nullptr) return;
    static uint32_t ordinal = 0, misses = 0, carpetBinds = 0, carpetEvents = 0;
    ++ordinal;
    const bool carpet = IsTempleCarpet(metadata);
    // Up to 4096 misses and 256 carpet samples. Repeated carpet hits are
    // sampled every 120 imports; allocation/reimport events are always kept.
    if (carpet) {
        if (carpetEvents >= 256) return;
        ++carpetBinds;
        if (hit && carpetBinds > 1 && carpetBinds % 120 != 0) return;
        ++carpetEvents;
    } else if (hit || misses >= 4096) {
        return;
    } else {
        ++misses;
    }
    ...
    if (metadata->resource != nullptr) {
        const auto& resource = metadata->resource;
        path = resource->GetInitData()->Path.c_str();
        const size_t bytes = std::min<size_t>(resource->ImageDataSize, carpet ? 4096 : 64);
        if (resource->ImageData != nullptr) {
            for (size_t b = 0; b < bytes; ++b) hash = (hash ^ resource->ImageData[b]) * 16777619u;
        }
    }
    const auto& t = rdp->texture_tile[tile];
    const auto& loaded = rdp->loaded_texture[t.tmem_index];
    const auto& prim = rdp->prim_color;
    const auto& env = rdp->env_color;
    fprintf(trace, "%lu %s id=%lu unit=%d tile=%d address=%p fmt=%u siz=%u bytes=%lu stride=%lu/%lu tileline=%lu "
                   "asset=%ux%u type=%u flags=%lu sample=%08lx combine=%016llx prim=%u,%u,%u,%u env=%u,%u,%u,%u replacement=%d %s\n",
            (unsigned long)ordinal, hit ? "HIT" : "MISS", (unsigned long)textureId, unit, tile, address,
            t.fmt, t.siz, (unsigned long)loaded.size_bytes, (unsigned long)loaded.line_size_bytes,
            (unsigned long)loaded.full_image_line_size_bytes, (unsigned long)t.line_size_bytes,
            metadata->width, metadata->height, (unsigned)metadata->type, (unsigned long)loaded.tex_flags,
            (unsigned long)hash, (unsigned long long)rdp->combine_mode,
            prim.r, prim.g, prim.b, prim.a, env.r, env.g, env.b, env.a, replacement, path);
    fflush(trace);
}
#endif

void Interpreter::ImportTexture(int i, int tile, bool importReplacement) {
    Soh3dsProfileScope textureProfile(Soh3dsProfileSection::Texture);
...
#ifdef __3DS__
    TraceTextureImport(mRdp, metadata, i, tile, origAddr, mRenderingState.mTextures[i]->second.texture_id,
                       cacheHit, importReplacement);
#endif
    if (cacheHit) {
        return;
    }
    ...
    uint32_t resBits = 0; // the archive's bits per texel, 0 = unknown/other
    {
    ...
    // SoH-3DS: the resource declares its own format, but so does the display
    // list, and each is wrong somewhere in OoT's archive:
    // - OoT's nine title effect masks (objects/object_mag/gTitleEffectMask*Tex)
    //   are Grayscale4bpp 64x64 in the archive yet arrive with the tile
    //   claiming CI/8b (stale), so 2048 bytes of intensity got read as palette
    //   indices: 67 arbitrary colours drawn as opaque discs over the logo.
    // - Child Zelda's 32x32 eye/mouth textures (object_zl4 gChildZeldaEye*Tex)
    //   are tagged Grayscale8bpp in the archive, but the head DL loads a
    //   256-entry TLUT and draws them as CI/8b, which is what the ROM does;
    //   decoding them as intensity painted palette indices as brightness
    //   (black face with white eye blobs in the intro ride).
    // The bit depth is the tell: a stale tile disagrees with the resource's
    // depth (4bpp asset, 8b tile), a mislabelled resource agrees with it
    // (8bpp both). The TLUT mode bits cannot decide it - this archive's
    // gsDPSetTextureLUT words carry the unshifted value 2, not G_TT_RGBA16.
    const uint32_t tileBits = 4u << siz;
    if (fmt == G_IM_FMT_CI && resBits != 0 && resBits != tileBits) {
...
void Interpreter::GfxSpVertex(size_t n_vertices, size_t dest_index, const F3DVtx* vertices) {
    Soh3dsProfileScope vertexProfile(Soh3dsProfileSection::Vertex);
...
// vertices. Runs once per state change (memoized on TriStateKey); the body is
// the upstream per-triangle code, with the per-vertex UV arithmetic folded
// into one multiply-add per axis at the end.
void Interpreter::DeriveTriState() {
    Soh3dsProfileScope triangleStateProfile(Soh3dsProfileSection::TriangleState);
    ...
    // shader_id participates in key comparison even though GenerateCC reads
    // the shader selection from options. Do not key the cache on stack bytes.
    ...
    ts.prg = prg;
    ts.tm = tm;
    ts.use_alpha = use_alpha;
    ts.use_fog = use_fog;
    ts.use_blend_color = use_blend_color;
    ts.use_grayscale = use_grayscale;
    ts.linear_filter = (mRdp->other_mode_h & (3U << G_MDSFT_TEXTFILT)) != G_TF_POINT;
    mRapi->ShaderGetInfo(prg, &ts.numInputs, ts.usedTextures);
    mClipParameters = mRapi->GetClipParameters();
    ...
    for (int t = 0; t < 2; t++) {
        ...
        if (tile.shifts != 0) {
            scaleS = tile.shifts <= 10 ? 1.0f / (1 << tile.shifts) : (float)(1 << (16 - tile.shifts));
        }
        if (tile.shiftt != 0) {
        ...
        ts.vAdd[t] = -tile.ult * 0.25f * invH;
        ts.halfU[t] = ts.linear_filter ? 0.5f * invW : 0.0f;
        ts.halfV[t] = ts.linear_filter ? 0.5f * invH : 0.0f;
...
bool gfx_set_timg_handler_rdp(F3DGfx** cmd0) {
    ...
    if ((i & 1) != 1) {
        ...
        if (gfx_check_image_signature(imgData) == 1) {
            // SoH-3DS: the archive keys resources by CRC64(path), so the
            // "__OTR__"-stripped path hashes straight into mTextureByHash and
            // shares the SETTIMG_OTR_HASH memo (weak, eviction-safe). Hashing
            // ~50 chars is far cheaper than the resource manager's string
            // copy + mutex + cache probe this replaced (~20 us per SETTIMG).
            const uint64_t pathHash = CRC64(imgData + 7);
            std::shared_ptr<Fast::Texture> tex;
            auto memo = gfx->mTextureByHash.find(pathHash);
            if (memo != gfx->mTextureByHash.end()) {
                tex = std::static_pointer_cast<Fast::Texture>(memo->second.lock());
            }
            if (tex == nullptr) {
                tex = std::static_pointer_cast<Fast::Texture>(gfx->ResolveResourceCached(imgData));
                if (tex != nullptr) {
                    gfx->mTextureByHash[pathHash] = tex;
                }
            }
...
bool gfx_set_timg_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    // SoH-3DS: memoized by hash (see mTextureByHash). An expired entry means the
    // resource was evicted; fall through to the full resolution and re-memoize.
    std::shared_ptr<Fast::Texture> texture;
    auto memo = gfx->mTextureByHash.find(hash);
    if (memo != gfx->mTextureByHash.end()) {
        texture = std::static_pointer_cast<Fast::Texture>(memo->second.lock());
    }
    if (texture == nullptr) {
        const char* fileName =
        ...
        gfx->mTextureByHash[hash] = texture;
    }
...
void Interpreter::Run(Gfx* commands, const std::unordered_map<Mtx*, MtxF>& mtx_replacements) {
    ...
    mRapi->StartFrame();
#ifdef __3DS__
    Soh3dsProfileScope displayListProfile(Soh3dsProfileSection::DisplayList);
#endif
    mRapi->StartDrawToFramebuffer(mRendersToFb ? mGameFb : 0, (float)mCurDimensions.height / mNativeDimensions.height);
    mRapi->ClearFramebuffer(true, true);
    mRdp->viewport_or_scissor_changed = true;
...
extern "C" void gfx_shader_cache_clear() {
    ...
    instance->mTriState.valid = false; // comb pointer dies with the pool
