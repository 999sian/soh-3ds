...
class TestArchive final : public Ship::Archive {
  public:
    ...
    void Reserve(size_t expectedFileCount) {
        ReserveIndex(expectedFileCount);
    }
...
// ============================================================
// Archive::ReserveIndex
// ============================================================

TEST(Archive, ReserveIndexPreallocatesWithoutChangingContents) {
    TestArchive arch("ram://arch", {
                                       { "a.bin", "a" },
                                       { "b.meta", "b" },
                                   });
    ...
    auto list = arch.ListFiles();
    ASSERT_NE(list, nullptr);
    EXPECT_GE(list->bucket_count(), 128u);
    EXPECT_TRUE(list->empty());

    arch.Load();
    EXPECT_EQ(arch.ListFiles()->size(), 2u);
    EXPECT_TRUE(arch.HasFile("a.bin"));
    EXPECT_TRUE(arch.HasFile("b.meta"));
}
