...
public:
    ...
    ~Companion();
    void SetLowMemoryMode(bool enabled) { gLowMemoryMode = enabled; }
    bool IsLowMemoryMode() const { return gLowMemoryMode; }
    void SetSetupProgress(std::function<bool(const char*, size_t, size_t)> callback) { gSetupProgress = std::move(callback); }
    ...
    std::optional<std::string> GetEnumFromValue(const std::string& key, int32_t id);
...
private:
    ...
    BinaryWrapper* gCurrentWrapper = nullptr;
    bool gLowMemoryMode = false;
    size_t gSetupFilesDone = 0, gSetupFilesTotal = 0;
    std::function<bool(const char*, size_t, size_t)> gSetupProgress;
    std::unordered_map<std::string, size_t> gRetainedDependencies;
    std::unordered_map<std::string, std::vector<std::string>> gDependencyEdges;
    void SetupCheckpoint(const char* stage);
    void ReleaseFileGroup();
