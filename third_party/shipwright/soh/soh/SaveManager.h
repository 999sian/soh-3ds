...
class SaveManager {
  public:
    ...
    bool SaveFile(int fileNum);
    bool SaveSection(int fileNum, int sectionID, bool threaded);
  ...
  private:
    ...
    bool SaveFileThreaded(int fileNum, SaveContext* saveContext, int sectionID);
...
// True only when an inline save has completed; queued desktop saves return false.
bool Save_SaveFile(void);
