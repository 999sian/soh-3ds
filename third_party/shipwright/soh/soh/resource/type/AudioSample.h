...
class AudioSample : public Ship::Resource<Sample> {
  public:
    ...
    // Own buffers independently of the engine-facing pointers, including while a load unwinds.
    void AllocateSampleData(size_t bytes, bool custom = false);
    void AllocateBookData(size_t entries);
    void MarkUnavailable() noexcept;
#if defined(__3DS__)
    static constexpr size_t MaxSampleBytes = 8 * 1024 * 1024;
#else
    static constexpr size_t MaxSampleBytes = 512 * 1024 * 1024;
#endif
  ...
  private:
    ...
    std::unique_ptr<int16_t[]> bookData;
    size_t decodedBudgetBytes = 0;
};
}; // namespace SOH