...
#ifdef __3DS__
// SoH-3DS: provided by the citro3d backend; marks a framebuffer id as the bottom LCD.
void Soh3dsSetBottomScreenFramebuffer(int fbId) __attribute__((weak));
void Soh3dsSetBottomScreenZoomFramebuffer(int fbId) __attribute__((weak));
// Fast3D framebuffer that IS the bottom screen (320x240, fixed 4:3). Game code
// brackets what should appear there with gsSPSetFB(gBottomScreenFrameBuffer).
// The Zoom id is the same LCD with Soh3dsSetBottomPassZoom applied.
s32 gBottomScreenFrameBuffer = -1;
s32 gBottomScreenZoomFrameBuffer = -1;
#endif
...
void FB_CreateFramebuffers(void) {
    ...
    if (gBottomScreenFrameBuffer == -1) {
        gBottomScreenFrameBuffer =
            gfx_create_framebuffer(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT, false, true);
        if (Soh3dsSetBottomScreenFramebuffer != NULL) {
            Soh3dsSetBottomScreenFramebuffer(gBottomScreenFrameBuffer);
        }
    }
    if (gBottomScreenZoomFrameBuffer == -1) {
        gBottomScreenZoomFrameBuffer =
            gfx_create_framebuffer(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT, false, true);
        if (Soh3dsSetBottomScreenZoomFramebuffer != NULL) {
            Soh3dsSetBottomScreenZoomFramebuffer(gBottomScreenZoomFrameBuffer);
        }
    }
#endif
}
