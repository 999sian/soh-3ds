...
#include "SaveFileTransaction.h"

static bool RecoverConsoleSave(const std::filesystem::path& path) {
    if (SaveFileTransaction::Recover(path.string())) return true;
    SPDLOG_ERROR("Save recovery failed: {}. Recovery files have been retained.", path.string());
    return false;
}

static bool WriteConsoleSave(const std::filesystem::path& path, const nlohmann::json& data) {
    if (SaveFileTransaction::Write(path.string(), data.dump(1))) return true;
    SPDLOG_ERROR("Save write failed: {}. Previous save and recovery files have been retained.", path.string());
    SohGui::RegisterPopup("Save failed", "The save could not be completed. Previous save and recovery files have "
                         "been retained. Check the SD card's free space and try again.");
    return false;
}

static bool ArchiveConsoleSave(const std::filesystem::path& source, const std::string& destination) {
    if (SaveFileTransaction::Copy(source.string(), destination) && SaveFileTransaction::Delete(source.string()))
        return true;
    SPDLOG_ERROR("Save backup failed: {}. The source save has been retained.", source.string());
    return false;
}
#endif
...
#if defined(__WIIU__) || defined(__SWITCH__) || defined(__3DS__)
    // Recover before testing existence: interruption may leave only a backup
    // and a completely written pending generation, with no primary filename.
    if (!RecoverConsoleSave(sGlobalPath)) return;
#endif
...
#if defined(__WIIU__) || defined(__SWITCH__) || defined(__3DS__)
        if (!RecoverConsoleSave(GetFileName(fileNum))) continue;
#endif
...
void SaveManager::StartupCheckAndInitMeta(int fileNum) {
    ...
    if (metaSaveBlock["sections"].contains("randomizer")) {
        if (!metaSaveBlock.contains("fileType") || metaSaveBlock["fileType"] == FILE_TYPE_SAVE_VANILLA) {
            ...
            saveMtx.lock();
#if defined(__WIIU__) || defined(__SWITCH__) || defined(__3DS__)
            const bool saved = WriteConsoleSave(GetFileName(fileNum), metaSaveBlock);
#else
            std::ofstream output(GetFileName(fileNum));
            output << metaSaveBlock.dump(1);
            output.close();
            const bool saved = !output.fail();
#endif
            saveMtx.unlock();
            if (!saved) return;
        }
        ...
        if (!(major == gBuildVersionMajor && minor == gBuildVersionMinor && patch == gBuildVersionPatch)) {
            ...
            if (!ArchiveConsoleSave(fileName, newFileName)) return;
#else
...
bool SaveManager::SaveFileThreaded(int fileNum, SaveContext* saveContext, int sectionID) {
    saveMtx.lock();
    ...
    std::filesystem::path fileName = GetFileName(fileNum);
#if defined(__SWITCH__) || defined(__WIIU__) || defined(__3DS__)
    if (!WriteConsoleSave(fileName, saveBlock)) return false;
#else
    ...
    output.close();
    if (output.fail()) {
        SPDLOG_ERROR("Save write failed: {}", tempFile.string());
        return false;
    }
...
bool SaveManager::SaveSection(int fileNum, int sectionID, bool threaded) {
    ...
    (void)threaded;
    return SaveFileThreaded(fileNum, &gSaveContext, sectionID);
#else
    SaveContext* saveContext = new SaveContext;
    memcpy(saveContext, &gSaveContext, sizeof(gSaveContext));
    if (threaded) {
        smThreadPool->detach_task(std::bind(&SaveManager::SaveFileThreaded, this, fileNum, saveContext, sectionID));
        return false; // Queued, not yet committed.
    } else {
        return SaveFileThreaded(fileNum, saveContext, sectionID);
    }
#endif
}

bool SaveManager::SaveFile(int fileNum) {
    return SaveSection(fileNum, SECTION_ID_BASE, true);
}
...
#if defined(__WIIU__) || defined(__SWITCH__) || defined(__3DS__)
    WriteConsoleSave(sGlobalPath, globalBlock);
#else
...
void SaveManager::LoadFile(int fileNum) {
    ...
    std::filesystem::path fileName = GetFileName(fileNum);
#if defined(__WIIU__) || defined(__SWITCH__) || defined(__3DS__)
    if (!RecoverConsoleSave(fileName)) {
        saveMtx.unlock();
        return;
    }
#endif
    ...
    try {
        ...
        std::string newFileName =
            ...
            ("/file" + std::to_string(fileNum + 1) + "-" + std::to_string(GetUnixTimestamp()) + ".bak");
#if defined(__SWITCH__) || defined(__WIIU__) || defined(__3DS__)
        if (!ArchiveConsoleSave(fileName, newFileName)) {
            saveMtx.unlock();
            SohGui::RegisterPopup("Error loading save file", "The save could not be loaded or backed up. "
                                 "The original file and recovery files have been retained.");
            return;
        }
#else
        std::filesystem::rename(fileName, newFileName);
...
bool SaveManager::SaveFile_Exist(int fileNum) {
    try {
#ifdef __3DS__
        // Inline saves cannot race this query. Restore an interrupted commit
        // so autosave eligibility does not mistake a recoverable slot for a deletion.
        if (!RecoverConsoleSave(GetFileName(fileNum))) return false;
#endif
...
void SaveManager::CopyZeldaFile(int from, int to) {
    ...
    ThreadPoolWait();
    {
        ...
        if (!SaveFileTransaction::Copy(GetFileName(from).string(), GetFileName(to).string())) {
            SPDLOG_ERROR("Save copy failed: slot {} to {}. Previous files retained.", from, to);
            SohGui::RegisterPopup("Save copy failed", "The save could not be copied. Previous save and recovery "
                                 "files have been retained. Check the SD card and try again.");
            return;
        }
    }
    GameInteractor::Instance->ExecuteHooks<GameInteractor::OnDeleteFile>(to);
#else
...
void SaveManager::DeleteZeldaFile(int fileNum) {
    ...
    {
        std::lock_guard<std::mutex> lock(saveMtx);
        if (!SaveFileTransaction::Delete(GetFileName(fileNum).string())) {
            SPDLOG_ERROR("Save deletion failed: slot {}. Previous save retained.", fileNum);
            return;
        }
    }
#else
...
extern "C" bool Save_SaveFile(void) {
    return SaveManager::Instance->SaveFile(gSaveContext.fileNum);
}
