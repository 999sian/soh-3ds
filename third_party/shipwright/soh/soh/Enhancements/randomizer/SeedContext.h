...
class Context {
  public:
    ...
    // Build spoiler data away from the live singleton, then publish it only
    // after parsing succeeds. File-select imports use this as a transaction.
    static std::shared_ptr<Context> CreateDetachedInstance();
    static void SetInstance(const std::shared_ptr<Context>& instance);
