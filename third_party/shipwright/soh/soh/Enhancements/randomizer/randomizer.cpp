...
#include "soh/Enhancements/dualscreen3ds/RandomizerMenu3DS.h"
...
void GenerateRandomizerImgui(std::string seed = "") {
    ...
    Soh3dsRandoProgress_SetStage(SOH3DS_RANDO_STAGE_SETTINGS);
#ifdef __3DS__
    ...
    Soh3dsRandoProgress_Finish(ctx->IsSeedGenerated());
    ...
    fprintf(stderr, "rando: gen done ok=%d, heap used=%u KiB (start %u KiB)\n", ctx->IsSeedGenerated() ? 1 : 0,
        ...
        Soh3dsRandoProgress_Finish(false);
        randoGenerating = false;
    }
#endif
}
...
bool GenerateRandomizer(std::string seed /*= ""*/) {
    bool expected = false;
    if (!randoGenerating.compare_exchange_strong(expected, true)) {
        return false;
    }
