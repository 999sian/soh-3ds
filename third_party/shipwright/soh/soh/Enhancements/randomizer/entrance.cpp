...
void EntranceShuffler::ApplyEntranceOverrides() {
    ...
    std::vector<std::pair<Entrance*, Entrance*>> validatedOverrides;
    validatedOverrides.reserve(entranceOverrides.size());
    for (size_t i = 0; i < entranceOverrides.size(); i++) {
        const EntranceOverride& entranceOverride = entranceOverrides[i];
        ...
        const auto entranceIt = entranceMap.find(entranceOverride.index);
        const auto overrideIt = entranceMap.find(entranceOverride.override);
        const auto validOptionalIndex = [](int16_t index) {
            return index == -1 || entranceMap.find(index) != entranceMap.end();
        };
        if (entranceIt == entranceMap.end() || entranceIt->second == nullptr || overrideIt == entranceMap.end() ||
            overrideIt->second == nullptr || !validOptionalIndex(entranceOverride.destination) ||
            !validOptionalIndex(entranceOverride.overrideDestination)) {
            throw std::invalid_argument("Spoiler file contains an unknown entrance index");
        }
        validatedOverrides.emplace_back(entranceIt->second, overrideIt->second);
    }

    // Validation is deliberately complete before the first graph mutation so
    // a malformed later entry cannot leave a partially applied world graph.
    for (const auto& [entrance, overrideEntrance] : validatedOverrides) {
        entrance->Disconnect();
