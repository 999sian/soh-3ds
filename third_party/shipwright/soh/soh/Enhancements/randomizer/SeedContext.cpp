...
std::shared_ptr<Context> Context::CreateDetachedInstance() {
    auto instance = std::make_shared<Context>();
    instance->GetLogic()->SetContext(instance);
    return instance;
}

void Context::SetInstance(const std::shared_ptr<Context>& instance) {
    mContext = instance;
}
...
void Context::AddExcludedOptions() {
    // This list belongs to the Context, while the area option registry belongs
    // to Settings. Rebuild the former on every initialization and keep the
    // existing name check below so repeated/detached initialization cannot
    // duplicate registry entries.
    everyPossibleLocation.clear();
