...
void CosmeticsUpdateTick() {
    // SoH-3DS: the full tick is ~800 CVar string lookups (every option's
    // rainbow flag plus every PATCH_GFX's changed flag) - measured as the
    // largest fixed per-tick cost on hardware. Nothing below changes anything
    // unless some option is in rainbow mode, so rescan for that once a second
    // and skip the rest while none is; the editor's toggle feels instant at
    // that cadence.
    static bool sAnyRainbow = false;
    static int sRescan = 0;
    if (--sRescan <= 0) {
        sRescan = 20;
        sAnyRainbow = false;
