...
const char* Interface_ReplaceSpecialCharacters(char text[]) {
    ...
    // SoH-3DS: malloc, not new[], so C callers can free() it (every caller
    // used to leak this - ~20 strings per frame from the bottom-screen UI).
    return strdup(textString.c_str());
}
