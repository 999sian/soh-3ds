...
bool HasPreset(const std::string& presetName, PresetSection section) {
    const auto preset = presets.find(presetName);
    return section >= PRESET_SECTION_SETTINGS && section < PRESET_SECTION_MAX && preset != presets.end() &&
           preset->second.apply[section] && preset->second.presetValues.contains("blocks") &&
           preset->second.presetValues["blocks"].contains(blockInfo[section].names[1]);
}
