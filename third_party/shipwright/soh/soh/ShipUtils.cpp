...
#if defined(__3DS__)
            // devkitARM's random_device falls back to a fixed PRNG seed.
            // Mix wall time with uptime once, so fresh launches vary even
            // when an emulator restarts its system tick at the same value.
            // This is gameplay randomness, not a cryptographic generator.
            uint64_t seed = svcGetSystemTick() ^ (osGetTime() * 0x9E3779B97F4A7C15ULL);
