...
#ifdef __3DS__
#include "soh/Enhancements/dualscreen3ds/QuickItems3DS.h"
#endif
...
#ifdef __3DS__
    if (index < 0) return ITEM_NONE;
    if (index >= 8) return Soh3dsQuickItems_ButtonItem(play, index);
#endif
...
#ifdef __3DS__
        for (int shortcut = 0; shortcut < 3; ++shortcut) {
            hasOnDpad |= Player_ItemIsItemAction(Soh3dsQuickItems_Item(shortcut), maskItemAction);
        }
#endif
...
#ifdef __3DS__
            for (int shortcut = 0; shortcut < 3; ++shortcut) {
                hasOnDpad |= Player_ItemIsInUse(this, Soh3dsQuickItems_Item(shortcut));
            }
#endif
            if (CVarGetInteger(CVAR_ENHANCEMENT("DpadEquips"), 0) != 0) {
                for (int buttonIndex = 0; buttonIndex < 4; buttonIndex++) {
...
#ifdef __3DS__
        if (i == ARRAY_COUNT(sItemButtons)) i = Soh3dsQuickItems_Button(play, true);
#endif
...
#ifdef __3DS__
            if (i == ARRAY_COUNT(sItemButtons)) i = Soh3dsQuickItems_Button(play, false);
#endif
            item = Player_GetItemOnButton(play, i);
