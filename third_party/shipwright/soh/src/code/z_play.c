...
#ifdef __3DS__
    /* Poll virtual item buttons even while paused, so a hold cannot survive
     * a modal screen or scene transition. Sampling precedes player update. */
    extern void Soh3dsQuickItems_Update(PlayState* play);
    Soh3dsQuickItems_Update(play);
#endif
...
#ifdef __3DS__
    // SoH-3DS dual screen: the pause pages (item/equip/quest/map) render into
    // the bottom-screen framebuffer; the top keeps the frozen backdrop that
    // Play_Draw already blitted. Only the normal pause states go down - the
    // Game Over / save prompt states (8..0x11) stay on the top screen.
    bool drawPauseOnBottom = gBottomScreenFrameBuffer != -1 && play->pauseCtx.debugState == 0 &&
                             play->pauseCtx.state >= 4 && play->pauseCtx.state <= 7 &&
                             CVarGetInteger(CVAR_ENHANCEMENT("DualScreen.Pause"), 1);
#endif
    if ((play->pauseCtx.state != 0) || (play->pauseCtx.debugState != 0)) {
#ifdef __3DS__
        if (drawPauseOnBottom) {
            OPEN_DISPS(play->state.gfxCtx);
            gsSPSetFB(POLY_OPA_DISP++, gBottomScreenFrameBuffer);
            CLOSE_DISPS(play->state.gfxCtx);
            gSoh3dsBottomPass = 1;
            if (Soh3dsSetBottomPassZoom != NULL) {
                Soh3dsSetBottomPassZoom(1.0f, 0.0f, 0.0f); // pages already fill 320x240
            }
        }
...
#ifdef __3DS__
        // Dual screen: tabs (items / map / settings) own the bottom LCD while
        // the pause pages are not on it (equips are refused while paused).
        // Before Interface_Draw so the Map tab's minimap (OVERLAY_DISP) lands
        // over the tab chrome (POLY_OPA_DISP).
        if (!drawPauseOnBottom) {
            Soh3dsBottomScreen_Draw(play);
        }
#endif
        Interface_Draw(play);
    }
...
bool Play_PerformSave(PlayState* play) {
