...
static void RunFrame() {
...
#ifdef __3DS__
            // SoH-3DS: per-phase tick timing, folded into the renderer's
            // heartbeat as tick=start/pad/update ms per 60 ticks. Ordinary
            // per-tick cost is what the player feels as game speed under load.
            extern s64 svcGetSystemTick(void);
            extern u64 gSoh3dsTickPhaseTicks[3];
            s64 t0 = svcGetSystemTick();
            Graph_StartFrame();
            s64 t1 = svcGetSystemTick();
            PadMgr_ThreadEntry(&gPadMgr);
            s64 t2 = svcGetSystemTick();
            Graph_Update(&runFrameContext.gfxCtx, gGameState);
            s64 t3 = svcGetSystemTick();
            gSoh3dsTickPhaseTicks[0] += t1 - t0;
            gSoh3dsTickPhaseTicks[1] += t2 - t1;
            gSoh3dsTickPhaseTicks[2] += t3 - t2;
#else
            Graph_StartFrame();
    ...
    // SoH-3DS: exit(0) here skips main.c's DeinitOTR, so static destructors ran
    // ndspExit/linearFree while the core-2 audio thread was still playing - a
    // crash on every quit-to-HOME on hardware. Join the workers first.
    DeinitOTR();
    exit(0);
}
