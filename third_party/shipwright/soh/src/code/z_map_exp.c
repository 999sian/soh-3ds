        ...
        switch (play->sceneNum) {
...
#ifdef __3DS__
                        // Dual screen: the whole minimap group (map, compass icons,
                        // marks) is zoomed about the map's centre by the renderer.
                        if (gSoh3dsBottomPass && Soh3dsSetBottomPassZoom != NULL) {
                            Soh3dsSetBottomPassZoom(CVarGetFloat(CVAR_ENHANCEMENT("DualScreen.MinimapScale"), 2.0f),
                                                    (dgnMiniMapX + 48.0f) / 160.0f - 1.0f,
                                                    1.0f - (dgnMiniMapY + 42.5f) / 120.0f);
                        }
#endif
            ...
            case SCENE_OUTSIDE_GANONS_CASTLE:
                if (!R_MINIMAP_DISABLED && CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) != 4) { // Not Hidden
                    ...
                    if (gSoh3dsBottomPass && Soh3dsSetBottomPassZoom != NULL) {
                        Soh3dsSetBottomPassZoom(CVarGetFloat(CVAR_ENHANCEMENT("DualScreen.MinimapScale"), 2.0f),
                                                (oWMiniMapX + gMapData->owMinimapWidth[mapIndex] * 0.5f) / 160.0f - 1.0f,
                                                1.0f - (oWMiniMapY + gMapData->owMinimapHeight[mapIndex] * 0.5f) / 120.0f);
                    }
#endif
