...
void Lights_GlowCheckPrepare(PlayState* play) {
    ...
    while (node != NULL) {
        ...
        if (node->info->type == LIGHT_POINT_GLOW) {
            ...
            wY = multDest.y * wDest;
#ifdef __3DS__
            // Match Fast3D's horizontal clip-space adjustment before the
            // depth-query bridge scales native coordinates to the viewport.
            wX *= (4.0f / 3.0f) / OTRGetAspectRatio();
#endif
...
void Lights_GlowCheck(PlayState* play) {
    ...
    while (node != NULL) {
        ...
        if (node->info->type == LIGHT_POINT_GLOW) {
            ...
            if ((multDest.z > 1.0f) && y >= shrink && y <= SCREEN_HEIGHT - shrink) {
#ifdef __3DS__
                // Readback is linear depth in steps of four, not N64's
                // compressed Z. Match UploadProjectionForActiveTarget's
                // 0.4999 clip-depth scale and DepthSnapshot3DS's 16-bit range.
                // The old 16352 scale biased far glows toward the camera,
                // enough to show through walls thousands of units nearer.
                wZ = (s32)(((multDest.z * wDest) * 0.4999f + 0.5f) * 65536.0f);
                zBuf = OTRGetPixelDepth(x, y);
#else
                wZ = (s32)((multDest.z * wDest) * 16352.0f) + 16352;
                zBuf = (OTRGetPixelDepth(x, y) * 4) >> 3;
#endif
