...
#ifdef __3DS__
    extern int Soh3dsQuickItems_UpdateBottle(PlayState* play, int item, int button);
    if (Soh3dsQuickItems_UpdateBottle(play, item, button)) return;
#endif
...
#ifdef __3DS__
        // SoH-3DS dual screen: the live minimap (and its dungeon marks) go to
        // the bottom screen's Map tab while playing; kaleido owns that screen
        // when paused and Minimap_Draw already skips those states.
        if (gBottomScreenZoomFrameBuffer != -1 && Soh3dsBottomScreen_Tab() == SOH3DS_TAB_MAP) {
            gsSPSetFB(OVERLAY_DISP++, gBottomScreenZoomFrameBuffer);
            gSoh3dsBottomPass = 1;
            gDPSetScissor(OVERLAY_DISP++, G_SC_NON_INTERLACE, 0, 24, 320, 200);
            Minimap_Draw(play);
            gSoh3dsBottomPass = 0;
            gsSPResetFB(OVERLAY_DISP++);
            /* Restore after switching targets: the interpreter must scale the
             * full-screen rectangle to the 400px top screen, not bottom 320. */
