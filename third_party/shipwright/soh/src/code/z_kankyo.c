...
#ifdef __3DS__
    // Match Fast3D's aspect adjustment; the bridge then scales this native
    // coordinate to the 400-pixel viewport, just as it does glow probes.
    return OTRGetPixelDepth((x - SCREEN_WIDTH / 2) * ((4.0f / 3.0f) / OTRGetAspectRatio()) + SCREEN_WIDTH / 2, y);
#else
...
void Environment_GraphCallback(GraphicsContext* gfxCtx, void* param) {
    ...
    f32 sunDepthX = D_8015FD7E;
    s32 sunDepthY = D_8015FD80;
#ifdef __3DS__
    // func_800C016C produces top-down window coordinates, but the 3DS depth
    // snapshot consumes GL Y-up coordinates like Lights_GlowCheck. Convert
    // only the probe: its existing five-pixel upward offset stays upward.
    sunDepthY = SCREEN_HEIGHT - sunDepthY;
    sunDepthX = (sunDepthX - SCREEN_WIDTH / 2) * ((4.0f / 3.0f) / OTRGetAspectRatio()) + SCREEN_WIDTH / 2;
#endif

    OTRGetPixelDepthPrepare(sunDepthX, sunDepthY);
    Lights_GlowCheckPrepare(play);

    D_8011FB44 = Environment_GetPixelDepth(D_8015FD7E, sunDepthY);
